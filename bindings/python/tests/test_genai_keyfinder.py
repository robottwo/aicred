...
def test_provider_instances_filtering():
    ...
    assert len(active_instances) == 2  # active_instance and anthropic_instance (default active)
...
def test_migration_utility():
    ...
    # This is a basic test since migrate_provider_configs is a placeholder
    instances = genai_keyfinder.migrate_provider_configs([])
    assert isinstance(instances, genai_keyfinder.ProviderInstances)
    assert instances.is_empty() is True
