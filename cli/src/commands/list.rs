...
/// Import the ProviderInstance from core library
use genai_keyfinder_core::models::ProviderInstance;
...
pub fn handle_list(verbose: bool, home: Option<String>) -> Result<()> {
    ...
    let config_dir = home_dir.join(".config").join("aicred");
    ...
    if !providers_dir.exists() {
        println!(
            "{}",
            "No configuration found. Run 'keyfinder scan --update' first.".yellow()
        );
        return Ok(());
    }
    ...
    let last_updated = providers
        .iter()
        .map(|p| p.instance.updated_at)
        .max()
    ...
    for provider_info in &providers {
        if verbose {
            println!(
                "\n{} {}",
                provider_info.name.cyan().bold(),
                format!("(instance: {})", provider_info.instance.id).dimmed()
            );

            // Show key information
            println!(
                "  Keys: {} total, {} active",
                provider_info.key_count, provider_info.active_key_count
            );

            // Show environment breakdown
            if !provider_info.keys_by_environment.is_empty() {
                let env_display: Vec<String> = provider_info
                    .keys_by_environment
                    .iter()
            ...
            // Show models
            if !provider_info.instance.models.is_empty() {
                let model_names: Vec<String> = provider_info
                    .instance
                    .models
                    .iter()
            ...
            if let Some(metadata_map) = &provider_info.instance.metadata {
                for (key, value) in metadata_map {
                    println!("  {}: {:?}", key, value);
                }
            }

            println!("  Active: {}", provider_info.instance.active);
            println!(
                "  Created: {}",
                provider_info
                    .instance
            ...
            let model_count = if provider_info.instance.models.is_empty() {
                "No models".to_string()
            } else {
                format!("{} models", provider_info.instance.models.len())
            };
...
mod tests {
    ...
    fn test_sanitize_provider_name_special_chars() {
        ...
        assert_eq!(
            sanitize_provider_name("AWS Bedrock v2.0"),
            "aws_bedrock_v2_0"
        );
    ...
    fn test_sanitize_provider_name_path_traversal() {
        ...
        assert_eq!(
            sanitize_provider_name("..\\..\\windows\\system32"),
            "windows_system32"
        );
            ...
            if let Some(file_stem) = path.file_stem().and_then(|s| s.to_str()) {
                match std::fs::read_to_string(&path) {
                    Ok(content) => {
                        match serde_yaml::from_str::<ProviderInstance>(&content) {
                            Ok(instance) => {
                                ...
                                let key_count = instance.keys.len() as u32;
                                let active_key_count = instance
                                    .keys
                                    .iter()
                                    .filter(|k| {
                                        k.validation_status
                                            == genai_keyfinder_core::models::ValidationStatus::Valid
                                    })
                                    .count()
                                    as u32;

                                let keys_by_environment =
                                    instance.keys.iter().fold(HashMap::new(), |mut acc, key| {
                                        let env_str = format!("{:?}", key.environment);
                                        *acc.entry(env_str).or_insert(0) += 1;
                                        acc
                                    });

                                // Use the provider_type from the instance as the display name
                            ...
                            Err(e) => {
                                eprintln!(
                                    "{} {}: {}",
                                    "Error parsing provider file:".red(),
                                    path.display(),
                                    e
                                );
                            }
                    ...
                    Err(e) => {
                        eprintln!(
                            "{} {}: {}",
                            "Error reading provider file:".red(),
                            path.display(),
                            e
                        );
                    }
        ...
        for (provider_name, provider_data) in providers {
            if let Some(name) = provider_name.as_str() {
                ...
                if let Some(api_key) = provider_data.get("api_key").and_then(|v| v.as_str()) {
                    // Convert provider data to new format - create a ProviderInstance
                    ...
                    default_key.validation_status =
                        genai_keyfinder_core::models::ValidationStatus::Unknown;
                    instance.add_key(default_key);
                    ...
                    if let Some(models) = provider_data.get("models").and_then(|v| v.as_sequence())
                    {
                        for model_str in models.iter().filter_map(|m| m.as_str()) {
                            instance.add_model(genai_keyfinder_core::models::Model::new(
                                model_str.to_string(),
                                model_str.to_string(),
                            ));
                        }
                    ...
                    write_secure_file(&provider_file_path, &serde_yaml::to_string(&instance)?)?;
                }
            }
        }
...
fn write_secure_file(path: &std::path::Path, content: &str) -> Result<()> {
    ...
    #[cfg(windows)]
    {
        ...
        if let Err(e) = output {
            eprintln!(
                "Warning: Could not set restrictive file permissions on Windows: {}",
                e
            );
