...
pub struct ProviderConfig {
    ...
    /// Schema version for configuration tracking.
...
/// Test-only structure for provider metadata in manifest files.
///
/// Note: This structure does not exist in the production codebase and is used
/// exclusively for testing the multi-file YAML storage system.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
...
/// Test-only structure for manifest files that track provider configurations.
///
/// Note: This structure does not exist in the production codebase and is used
/// exclusively for testing the manifest-based multi-file storage system.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct Manifest {
...
#[test]
fn test_legacy_format_removed() {
    // This test verifies that the old providers.yaml format is no longer supported
    // The legacy functionality has been removed as part of the directory structure change
