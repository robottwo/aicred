...
impl Scanner {
    ...
    #[must_use]
    pub const fn new(provider_registry: PluginRegistry) -> Self {
        Self {
            provider_registry,
    ...
    ///
    /// # Errors
    /// Returns an error if the home directory does not exist or if scanning fails.
    pub fn scan(&self, home_dir: &Path) -> Result<ScanResult> {
        ...
        let mut result =
            ScanResult::new(home_dir.display().to_string(), providers, scan_started_at);
