...
impl Provider {
    ...
    #[must_use] pub const fn new(name: String, provider_type: String, base_url: String) -> Self {
        Self {
            name,
            provider_type,
            base_url,
    ...
    #[must_use] pub fn with_documentation_url(mut self, url: String) -> Self {
        self.documentation_url = Some(url);
        self
    }

    /// Sets the rate limits for the provider.
    #[must_use] pub fn with_rate_limits(mut self, rate_limits: RateLimit) -> Self {
        self.rate_limits = Some(rate_limits);
        self
    }

    /// Sets the authentication methods for the provider.
    #[must_use] pub fn with_auth_methods(mut self, methods: Vec<AuthMethod>) -> Self {
