...
impl ProviderConfig {
    ...
    #[must_use] pub fn new(version: String) -> Self {
        let now = Utc::now();
        Self {
            keys: Vec::new(),
            models: Vec::new(),
    ...
    #[must_use] pub fn keys_by_environment(&self, env: &Environment) -> Vec<&ProviderKey> {
        self.keys
            .iter()
    ...
    #[must_use] pub fn keys_by_status(&self, status: ValidationStatus) -> Vec<&ProviderKey> {
        self.keys
            .iter()
    ...
    #[must_use] pub fn from_old_format(
