...
impl ProviderInstances {
    ...
    /// * `Err(String)` if an instance with the same ID already exists
    pub fn add_instance(&mut self, instance: ProviderInstance) -> Result<(), String> {
        if self.instances.contains_key(&instance.id) {
            return Err(format!("Provider instance with ID '{}' already exists", instance.id));
        }
    ...
    /// 
    /// # Returns
    /// * `Ok(())` if all instances are valid
    /// * `Err(String)` with validation errors for invalid instances
    ...
    pub fn merge(&mut self, other: ProviderInstances) {
...
mod tests {
    ...
    fn test_add_and_get_instance() {
        ...
        assert!(collection.add_instance(instance.clone()).is_err()); // Duplicate ID
    ...
    fn test_instances_by_type() {
        ...
        collection.add_instance(create_test_instance("openai1", "openai", true)).unwrap();
        collection.add_instance(create_test_instance("openai2", "openai", true)).unwrap();
        collection.add_instance(create_test_instance("anthropic1", "anthropic", true)).unwrap();
    ...
    fn test_active_instances() {
        ...
        collection.add_instance(create_test_instance("active1", "openai", true)).unwrap();
        collection.add_instance(create_test_instance("inactive1", "openai", false)).unwrap();
        collection.add_instance(create_test_instance("active2", "anthropic", true)).unwrap();
    ...
    fn test_instances_with_valid_keys() {
        ...
        collection.add_instance(create_test_instance("valid1", "openai", true)).unwrap();
        collection.add_instance(create_test_instance("invalid1", "openai", false)).unwrap();
    ...
    fn test_merge_collections() {
        ...
        collection1.add_instance(create_test_instance("test1", "openai", true)).unwrap();
        collection2.add_instance(create_test_instance("test2", "anthropic", true)).unwrap();
        collection2.add_instance(create_test_instance("test1", "groq", true)).unwrap(); // Same ID, should replace
        
        collection1.merge(collection2);
        
        assert_eq!(collection1.len(), 2);
        assert_eq!(collection1.get_instance("test1").unwrap().provider_type, "groq");
    }
}