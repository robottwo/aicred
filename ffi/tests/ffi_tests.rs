...
fn test_null_handling() {
    unsafe {
        ...
        let options = CString::new(r#"{"include_full_values": false}"#).unwrap();
        let result = aicred_scan(ptr::null(), options.as_ptr());
        ...
        let home = CString::new("/tmp/test").unwrap();
        let result = aicred_scan(home.as_ptr(), ptr::null());
        assert!(result.is_null());
...
fn test_free_valid_pointer() {
    unsafe {
        ...
        // The null check in aicred_free only protects against null pointers,
    ...
    let handles: Vec<_> = (0..5)
        .map(|i| {
            thread::spawn(move || unsafe {
                ...
                let result = aicred_scan(home.as_ptr(), options.as_ptr());
                assert!(!result.is_null() || !aicred_last_error().is_null());
